...
err = json.新解码器(六).解码(&配置)
		f.关闭()
如果呃！=无 {
恐慌（呃)
		}
配置.Z.驱动因素=制造([]零.司机,低输入联网(低入门网络的缩写)(配置.W)+低输入联网(低入门网络的缩写)(配置.s))
为我，w :=范围配置.W {
配置.Z.驱动程序[i] = w
		}
为我，s :=范围配置.S {
配置.Z.驱动程序[i+低输入联网(低入门网络的缩写)(配置.W)] = s
		}
